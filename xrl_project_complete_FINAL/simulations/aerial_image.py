...
class XRayMask:
    ...
    def __init__(self, 
        ...
        self.absorber_key = absorber_material  # Store the key
        self.absorber = MATERIALS[absorber_material]
        self.absorber_thickness = absorber_thickness

        self.membrane_key = membrane_material  # Store the key (FIX!)
...
# In aerial_image.py, add the following function before 'if __name__ == "__main__":'

def parameter_sweep_absorber_material(mask_base: XRayMask,
                                      energy_kev: float = 1.5,
                                      gap: float = 10.0,
                                      absorber_materials: list = ['Ta', 'W', 'Au']) -> dict:
    """
    Sweep different absorber materials and calculate contrast/transmission.
    """
    results = {}
    
    print(f"\nAbsorber Material Sweep at {energy_kev} keV, {gap} μm gap:")
    print("-" * 50)
    print(f"{'Material':<10} {'Contrast':<10} {'Transmission':<15}")
    print("-" * 50)
    
    for material in absorber_materials:
        # Create a new mask for each material
        mask = XRayMask(
        ...
        print(f"{material:<10} {contrast:<10.3f} {transmission:<15.4e}")
