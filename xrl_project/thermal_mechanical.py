...
class MembraneMechanics:
    ...
    def thermal_deflection(self, delta_T: float) -> float:
        """
        Calculate deflection from thermal gradient through thickness.
        
        Args:
            delta_T: Temperature difference across thickness (K)
        ...
        # Bimetallic strip approximation
        # w_max ≈ 3 * alpha * ΔT * a^2 / (2 * t)
        w_max = 3 * alpha * delta_T * a**2 / (2 * t)
...
class ThermalAnalysis:
    ...
    def steady_state_temperature_rise(self,
                                     absorbed_power: float,
                                     convection_coefficient: float = 10.0) -> float:
        """
        Estimate steady-state temperature rise.
        
        Simplified model: heat transfer by convection and conduction.
        
        Args:
            absorbed_power: Power absorbed in membrane (W)
            convection_coefficient: h (W/(m²·K))
        
        Returns:
            Temperature rise above ambient (K)
        """
        # Surface area for convection (both sides)
        A_surface = 2 * (self.membrane.size**2)  # m²
        
        # Heat transfer by convection
        Q_conv = convection_coefficient * A_surface
        
        # Temperature rise
        delta_T = absorbed_power / Q_conv
...
def exposure_scenario_analysis(membrane: MembraneMechanics,
    ...
    for i, power in enumerate(beam_power_range):
        ...
        delta_T = thermal.steady_state_temperature_rise(absorbed)
        stress = membrane.thermal_stress(delta_T)
        deflection = membrane.thermal_deflection(delta_T / 10)  # Gradient ~10% of rise
        
        results['temperature_rise_K'][i] = delta_T
        results['thermal_stress_MPa'][i] = stress
        results['deflection_um'][i] = deflection
        
        if i % 5 == 0:  # Print every 5th point
            print(f"{power:<12.4f} {delta_T:<12.2f} {stress:<15.2f} {deflection:<15.3f}")
...
def material_comparison(thickness: float = 2.0,
    ...
    for mat_name, mat_props in MEMBRANE_MATERIALS.items():
        ...
        delta_T = thermal.steady_state_temperature_rise(absorbed)
        stress = membrane.thermal_stress(delta_T)
        deflection = membrane.thermal_deflection(delta_T / 10)
        ...
        print(f"{mat_name:<15} {delta_T:<12.2f} {stress:<15.2f} {deflection:<15.3f}")
...
if __name__ == "__main__":
    ...
    for mat_name in materials:
        ...
        delta_T = thermal.steady_state_temperature_rise(absorbed)
        deflection = mem.thermal_deflection(delta_T / 10)
        deflections.append(deflection)
    
    axes[1, 1].bar(materials, deflections, color=['blue', 'red', 'green'], alpha=0.7)
