...
class MaterialProperties:
    ...
    def get_attenuation_coefficient(self, energy_kev: float) -> float:
        """
        Calculate mass attenuation coefficient using power law approximation.
        
        For X-rays in 0.5-5 keV range:
        μ/ρ ≈ C * Z^4 / (A * E^3)
        
        Returns: μ (1/cm)
        """
        C = 9.8e6  # empirical constant for this energy range
        mu_over_rho = C * (self.atomic_number**4) / (self.atomic_mass * energy_kev**3)
...
def parameter_sweep_absorber_thickness(mask_base: XRayMask,
    ...
    for i, thickness in enumerate(thicknesses):
        mask = XRayMask(
            absorber_material=mask_base.absorber_material,
            absorber_thickness=thickness,
            membrane_material=mask_base.membrane_material,
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(output_dir, 'aerial_image_example.png')
